    ...
    def train_eval(self, data, config, use_saved_embeddings=True):
        ...
        while (epoch < self.config['max_epochs']):
            ...
            if val_cor[1][0] + val_cor[0][0] > best_val_cor[1][0] + best_val_cor[0][0]:
                ...
                # print 'valid data: '
                # calculate_model_correlation(index_val/4, index_test/4, config, score=model_val_out, order=order)
                ...
                # print 'test data: '
                # calculate_model_correlation(index_test/4, len(data), config, score=model_out_test, order=order)
        ...
        print 'bleu on model level valid: '
        # calculate_model_correlation(index_val / 4, index_test / 4, config)
        calculate_sentence_correlation(index_val / 4, index_test / 4, config)
        print 'bleu on model level test: '
        # calculate_model_correlation(index_test / 4, len(data), config)
        calculate_sentence_correlation(index_test / 4, len(data), config)
